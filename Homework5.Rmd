...
fun = function(df) {
  prop.test(n.x ~ n.y, joined = df), 
}

##the following is the closest option yet:
...
out
  ...
  #select(estimate, conf.int) #I get error "C stack usage 7970144 is too close to the limit"
all_city_prop
...
new = all_city_prop %>% 
  ...
  reduce(cbind) %>%
  unnest(output)
  select(estimate)      #this gives a list of 450 with no obs/var labels..ughhh
  filter(   )          #try to select the 4th and 6th value in each 9-item-long list  
  #reduce(cbind) #this further orders it but in a more confusing way that makes it almost impossible to select from 
